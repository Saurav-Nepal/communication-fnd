...
>(({ className, children, withScrollableModal, ...props }, ref) => (
    <>
        <DialogPrimitive.Overlay
            ref={ref}
            className={cn(
                'fixed inset-0 z-50 flex justify-center bg-black/75  overflow-y-auto transition-all duration-100 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=open]:fade-in',
                className
            )}
            {...props}
        >
            ...
            <DialogPortal>
                <DialogOverlay withScrollableModal={withScrollableModal}>
                    <DialogPrimitive.Content
                        ref={ref}
                        className={cn(
                            'relative z-50 flex flex-col w-full max-h-screen bg-dialog overflow-clip shadow-modal modal-dialog animate-in data-[state=open]:fade-in-90 sm:rounded-lg sm:zoom-in-90 sm:self-center',
                            {
                                'max-h-none sm:self-start mt-5':
                                    withScrollableModal,
                            },
                            className,
                    ...
                    >
                        {title && (
                            <div className='p-4 border-b row-flex bg-dialog-header'>
                        ...
                        {isCloseable && !hideCloseIcon && (
                            <DialogPrimitive.Close
                                className={cn(
                                    'dialog-close',
                                    'absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent',
                                    closeClassName
                                )}
                            >
                                <X className='w-4 h-4' />
