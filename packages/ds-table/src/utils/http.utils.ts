...
function StoreNetInfo(state: any) {
...
function ProcessDataAfterApiCall(result: any): any {
    ...
    if (ErrorStatus.includes(status || response?.status)) {
        if (!alreadyServerDead) {
            let responseData = (ErrorResponses as ObjectDto)[status];
