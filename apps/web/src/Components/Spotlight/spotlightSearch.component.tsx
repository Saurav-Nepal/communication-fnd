...
export const SpotlightSearch = ({
    ...
    const {
        ...
        setSearchText,
        setSearchType,
    } = useSpotlightSearch(menus, queries, {
        onOpen: () => {
    ...
    // Detect the os and handle the keypress
    const handleKeyPress = useCallback(
        (e: KeyboardEvent) => {
            ...
            if (e.key === 'k' && hasPressed) {
                e.preventDefault();
                handlers.open_spotlight();
            }
            if (e.key === 'h' && hasPressed) {
                e.preventDefault();
        ...
        return (
            <CommandGroup heading='History'>
                {filteredItems.map((path, index: number) => {
                    ...
                    const displaySubtitle = pathName.split('?')[0];
        ...
        >
            <div className='relative'>
                ...
                {IsValidString(searchType) && (
                    <div className='absolute items-center gap-2 pr-8 -translate-y-1/2 right-2 top-1/2 text-base-secondary row-flex'>
            ...
            <CommandList className='sm:max-h-[400px]' ref={commandListRef}>
