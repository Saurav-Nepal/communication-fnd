...
import { SHOW_TOAST } from '@/constants/state.constants';

import { SubscribeToEvent, UnsubscribeEvent } from './stateManager.utils';
...
export class Toast {
     ...
     */
    static handleToastBroadcast({ type, description, delay, ...props }: any) {
        this.notification({
            type: type,
...
const template = ({ data, toastProps }: ToastContentProps<ToastDataProps>) => {
    ...
    return (
        <div
            className={cn(
                'toast-content text-base-primary dark:text-neutral text-sm font-rubik flex flex-col gap-2',
                templateClassName
            )}
        >
