...
const LoginModule = () => {
    ...
    if (no_business) {
        return '';
        // return <VendorSignOut />;
    }

    if (loading) {
        return (
            <PageLoader
                className='bg-[var(--dashboard-bg-color)]'
                screenHeight
            />
        );
    }
...
const LoginScreen = () => {
    const { mobile, email } = useFetchParams();
...
const LoginMainSection = ({ data, onMobile, onEmail }: any) => {
    ...
    return (
        <form
            ...
            noValidate
            className='justify-between gap-6 col-flex'
        >
...
const EmailLoginSection = ({ email, resetLogin = () => {} }: any) => {
    ...
    return (
        <FormBuilder
            className='flex-1 h-full justify-between !pb-0'
            formSchema={schema}
